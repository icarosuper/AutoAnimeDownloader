...
| `adoptCoveredEpisodes(...)` | (`coverage.go`) Devolve os `EpisodeStruct` dos episódios pendentes que um pack já em disco cobre no eixo absoluto — o registro aponta para o hash existente, sem download e sem Nyaa, com a faixa convertida para a numeração local de quem adota (decisions.md #78) |
| `findCoveringPack(saved, seriesIndex, hashSet, series, absEp)` | (`coverage.go`) O registro de pack, em **qualquer** `anime_id`, cuja faixa declarada (`hasDeclaredRange`) contém `absEp`, **e o offset do dono** (é ele que converte a faixa). Quatro portas: faixa conhecida, torrent vivo na sessão, mesma `Series.Key`, e o anime pedido não ser filme |
| `packDisplayName(title, start, end)` | (`episodes.go`) Nome exibido de um registro de pack, com o começo clampado em 1 — a faixa gravada pode começar em zero ou abaixo (#79). Usado por `assignBatches` e pela adoção |
...
| `packAxis` / `newPackAxis(anime, seriesIndex, totalEpisodes)` | As hipóteses de numeração de um pack, como quanto somar ao número **local** da entrada: relativa (0, sempre primeira), contínua pela season (`ComputeEpisodeOffset`) e absoluta pela série (`anilist.Series.Offset`). O zero-value = só a relativa, o comportamento antigo (decisions.md #79) |
| `packAxis.localRange(info, episode)` | Converte a faixa declarada no nome do pack para a numeração local da entrada. Só concorre hipótese que cobre `episode`; entre as que cobrem ganha a que faz o pack **terminar** no último episódio da entrada (desempate por contagem, `sources.md`). O começo convertido pode ser ≤ 0 |
| `packCandidate` struct | Um pack (`torrent`) e a faixa que ele cobre (`info`), na numeração que o **grupo** usou — quem converte para a da entrada é `packAxis.localRange`. `resolved` marca que a faixa já foi buscada |
| `packSet` / `newPackSet(results, detail)` | Os packs candidatos com a faixa de cada um, resolvida **sob demanda**: do nome quando o nome a traz, senão da lista de arquivos da página de detalhe (`detail`, o `packRange` do `nyaaSearcher`). Sob demanda porque a maioria das buscas não chega a precisar do detalhe — ver [decisions.md #84](decisions.md#84-a-cobertura-de-um-pack-sem-faixa-no-nome-vem-da-lista-de-arquivos-e-não-da-suposição-de-que-ele-cobre-tudo) |
| `packSet.rangeOf(i)` | A faixa do pack `i`, buscando o detalhe na **primeira** vez que ela é pedida para um pack sem faixa no nome. Memoriza inclusive a falha — sem isso `assignBatches` (que varre os packs uma vez por episódio) viraria uma requisição por episódio |
| `packSet.covering(axis, episode)` | First pack that covers `episode` em alguma hipótese do `axis`, **e** a faixa dele já convertida para a numeração local. Pack cuja faixa continua desconhecida **depois** do detalhe conta como cobrindo tudo (um Nyaa fora do ar não pode zerar os packs) — mas o que se grava nesse caso é faixa zero, ver `assignBatches` |
| `maxPackDetailFetches` const | `3` — quantas páginas de detalhe um anime pode custar por passe |
| `pickBatches(candidates, axis, firstPending, windowEnd)` | Minimum set of packs (already sorted/filtered) that covers `[firstPending, windowEnd]`, como um `*packSet` já resolvido (não busca mais detalhe nenhum). Batch eligibility comes from this — the search **result**, not anime metadata |
| `assignBatches(animeTitle, axis, episodes, batches)` | Gives each episode its own pack's magnet; truncates at the first uncovered episode (prefix cut — the schedule is ascending and picked packs are contiguous from `firstPending`) so nothing falls through to the single-episode fallback. Grava a faixa **convertida**; o nome exibido corta o começo em 1. Faixa que nem o nome nem os arquivos resolveram grava **zero**, não `1..total`: as duas fontes falharam e "não sei" é o único registro honesto (decisions.md #84) |
| `hasDeclaredRange(ep)` | Se o registro traz a faixa do pack. O sentinela de "desconhecida" é o **fim** (`BatchEnd`): começo 0 ou negativo é faixa legítima ("00 ~ 12", ou pack de season gravado sob um cour posterior) |
...
| `nyaaSearcher.packRange(tr)` | O quarto membro do seam de busca (`search.go`), ao lado de `searchAnime`/`searchSingleEpisode`/`searchMovie`: lê a faixa real de um pack na página de detalhe (`nyaa.PackFileRange(tr.DetailURL)`). Separado das buscas porque não é busca — roda depois do filtro e da ordenação, e só para o pack que o nome não resolve |
...
Standalone animes (animes tracked without being in any AniList list). `appendStandaloneAnimes` and `DownloadStandaloneAnime` are listed in the package symbol table above; both are documented in [decisions.md #49](decisions.md#49-anime-avulso-acompanhado-pela-lista-é-o-snapshot-que-o-daemon-processa-e-downloadstandaloneanime-nunca-chama-handlesavedepisodes).
...
- `searchIssue(...)` — a cascata de precedência dos três problemas de busca (ver decisions.md #81).
...
| `SetLastCheckError(err)` | **Limpa** `lastCheckReport`. Consequência: `SetLastCheckReport` tem de ser chamado depois do `SetLastCheckError(nil)` do fim do passe (ver decisions.md #82) |
...
- `refreshOrphanAnimes` — for animeMap entries with a known AnimeID that no account's fetch covered (current status fell outside the allowed sets), re-fetches cover/progress/blacklist in **two batch queries at most**: `anilist.GetAnimeInfoByIDs` (one per account per 50 ids), then `refreshStandaloneOrphans` → `anilist.GetMediaByIDs` for the ids no account tracks but the standalone file does. A failed refresh just logs a warning and leaves the anime as-is — it's never removed; on error the standalone fallback is skipped, since "no account tracks it" isn't a conclusion you can draw from a failed fetch. It was one request **per orphan per account** until 29/ago/2026, with a 5-way concurrency semaphore that capped parallelism and not the request count (decisions.md #65)
...
| `GetAnimeInfo(mediaId, usernames, priority)` | Fetches one anime by **media** id with full airing schedule, synonyms, and relations, querying each account and collapsing via `DedupeByMedia`. Returns `(nil, nil)` when no account tracks it — a normal state, not an error (used by `/animes/{id}/episodes` and `daemon.RunAnimeDebug`) |
| `GetAnimeInfoByIDs(mediaIds, usernames)` | Batch twin of `GetAnimeInfo`: one `mediaList(userName:, mediaId_in: [...])` query per account per 50 ids, same `DedupeByMedia` collapse across accounts and chunks. A media id no account tracks is simply **absent** from the returned map (not an error); the error means at least one account's request failed and does **not** invalidate the map. Not cached. This is what makes `refreshOrphanAnimes` affordable on every frontend poll (decisions.md #65) |
...
| `GetMediaByIDs(ids, priority)` | Batch twin of `GetMediaByID`: one `Page(media(id_in: [...]))` query per 50 ids instead of one request per id — the fix for the standalone loop that dominated the AniList budget (decisions.md #65). Returns `map[int]*MediaList` with **three** outcomes per id: a value (fetched or cached), an explicit `nil` (AniList doesn't know the id — `id_in` silently omits it), or **absent from the map** (couldn't be fetched; only this case comes with an error). The returned error does not invalidate the map — pages that succeeded are already in it. Shares `mediaByIDCache` with `GetMediaByID`, keyed **per id**, so only cache misses go on the wire |
| `GetMediaByID(mediaId, priority)` | Reads one anime by media id **without** going through any account's list — the primitive a standalone anime needs (`GetAnimeInfo` returns nil for it). Returns a **synthetic** `MediaList`: only `Media` filled, `Progress: 0`, `Status: ""`, `Id: 0` — the zeros are part of the contract, since `DedupeByMedia` keeps the LOWEST progress. Same fields as `getMediaListEntry` and `GetAnimeInfoByIDs` — literally the same `mediaByIDFields` block, so the three can't drift — because the search/selection path (`searchNyaaForAnime`, `searchNyaaForSingleEpisode`) depends on synonyms, relations and airing-schedule ids. Cached 60s per id |
...
| `sendAnilistRequest[T](query, variables, priority)` | Generic GraphQL POST helper. Every call declares a `Priority` — there is no default: the budget gate (`health.go`) refuses `PriorityDisposable` when AniList's remaining quota is low, and records `X-RateLimit-Remaining` from **every** response, errors included |
| `GetFrontendAnimeList(username, statuses)` | Lighter list query behind `GET /animes`. **Cached for 60s** per `username+statuses` and hands out a copy of the slice — the frontend polls this endpoint every 30s per open tab, which used to blow AniList's 30 req/min budget and 429 the daemon's own loop (decisions.md #46). `PriorityDisposable`: when the budget gate refuses it, it serves the **expired** cache entry rather than failing the poll |
| `GetCustomListsMap(username, statuses, priority)` | Minimal `id + customLists` query, cached 5min (30s when the response is empty) — see decisions.md #11 |
| `ttlCache[T]` | Tiny TTL map behind all three caches (`get`/`set`/`clear`, plus `getStale` — used only by the budget gate's fallback) |
...
O **gate de orçamento** também vive aqui, ao lado da saúde e pela mesma razão: os dois se
alimentam de toda resposta que passa por `sendAnilistRequest`. Ver decisions.md #72.

| Symbol | Purpose |
|--------|---------|
| `Priority` / `PriorityCritical` / `PriorityDisposable` | Criticidade declarada por chamada. Sem valor padrão de propósito: uma query nova não compila sem alguém decidir. Crítico = passe do daemon + ação de um clique com o usuário esperando; descartável = tráfego que se repete sozinho (poll de `/animes`, busca por tecla) |
| `ErrBudgetLow` | A recusa do gate. Nenhuma requisição saiu, e a saúde do pacote não muda |
| `recordBudget(header)` | Grava `X-RateLimit-Remaining` + o instante da leitura, em **toda** resposta (inclusive 4xx: erro consome cota igual) |
| `budgetAllows(priority)` | Crítico sempre passa; descartável passa se a leitura estiver vencida (`budgetValidity`, 60s — a saída obrigatória, senão o gate se auto-trava) ou acima de `budgetFloor` (10 de 30) |

### `src/internal/nyaa/nyaa.go`

| Symbol | Purpose |
|--------|---------|
| `TorrentResult` struct | `Name`, `MagnetLink`, `Seeders`, `Leechers`, `Episode*`, `Resolution*`, `Season*`, `Part*`, `Size`, `Fansub`, `IsBatch`, `DetailURL` (a página `/view/<id>` da linha, absoluta; só `ScrapNyaaForAnime` a preenche, e só `PackFileRange` a usa) |
...
| `SetMaxBatchTorrentSizeGB(gb)` / `batchTooBig(size)` | Pack size ceiling from `max_batch_torrent_size_gb`, pushed by `files.LoadConfigs` (same atomic+restore pattern). `ScrapNyaaForAnime` drops an oversized pack row **before** it counts toward `enoughCandidates`, so giant packs can't end the page descent ahead of the partial packs that fit — see [Decisions #80](decisions.md#80-o-teto-de-tamanho-de-pack-é-aplicado-duas-vezes-na-busca-do-nyaa-e-no-filtro-do-daemon). Default `0` (off); `Size == 0` passes, same rule as `daemon.filterBySize` |
...
| `ExtractBatchInfo(name)` | Exported version of `extractBatchInfo`: parses a pack's episode range (`StartEpisode`/`EndEpisode`/`Season`) out of the torrent name. `daemon.newPackSet` uses it as the **first** source of a pack's range. Contract: `EndEpisode == 0` means "unknown range", and the caller then asks `PackFileRange` |
| `PackFileRange(detailURL)` | A faixa que o pack **realmente** traz, lida da lista de arquivos da página de detalhe (`.torrent-file-list`), na mesma régua do nome. Pastas, não-vídeo e arquivo sem número legível (NCOP/NCED) ficam de fora; `(info, false)` quando o detalhe não responde ou nenhum arquivo dá o número. Medido: 19 de 24 packs não declaram faixa no nome — ver [decisions.md #84](decisions.md#84-a-cobertura-de-um-pack-sem-faixa-no-nome-vem-da-lista-de-arquivos-e-não-da-suposição-de-que-ele-cobre-tudo) |
| `IsVideoFile(name)` | Extensão de vídeo. Mora aqui, e não em `files`, porque os dois leem a mesma lista de arquivos de um pack (o Librarian depois de baixar, `PackFileRange` antes) e `files` já importa `nyaa` |
| `detailURL(href)` | Absolutiza o `href` da coluna de título (`/view/1323474`); vazio quando a linha não traz link |
| `fileListEntryName(s)` | O nome do arquivo numa linha da lista, sem o `span.file-size` que vem no mesmo `<li>` e sem os filhos de uma sublista |
...
| `ExtractEpisodeNumber(name)` | Exported: the episode number from a torrent name **or** from a file name inside a pack (Librarian rename, `TorrentFileResponse.episode`). Normalizes `_` to space first — every pattern in `reEpisodePatterns` needs whitespace around the number, and a group that separates with underscores (`[DB]Anime_-_01_(...).mkv`) matched none of them |
...
| `jaccardThreshold(queryLen)` | Minimum Jaccard similarity for a match, by query length: `0.8` for ≤3 tokens (keeps "SAO" from matching "SAO Alternative Gun Gale Online"), `0.4` for ≥4 (a fansub embedding both title translations inflates the union without meaning a wrong match) |
...
| File | Export | Purpose |
|------|--------|---------|
| `animeState.ts` | `deriveAnimeChip(anime, torrents, now, stalledSince)` | Ordered cascade → one chip state: blacklisted → downloading → no-seeds (torrent **incompleto** ≥10min at 0 peers — um torrent completo sem peers é seed ocioso, não travamento) → paused → awaiting-premiere → up-to-date → behind. Returns a **key plus raw numbers**, never rendered text, so it stays locale-free; the screen translates the key |
...
| `priorityPresets.ts` | `PRESETS`, `applyPreset(list, first)` | One-click codec presets for the Priorities screen (compatibility = H.264 first, space = AV1/HEVC first). `applyPreset` **promotes** the tokens of `first` that the list already has instead of replacing it, so a typo is a no-op and a hand-added token moves down instead of disappearing — see [decisions.md #70](decisions.md#70-resolução-canonicaliza-no-extrator-e-os-presets-de-codec-são-reordenação-no-frontend) |
...
| `stallTracker.ts` | `stallTracker` | Per torrent hash, the timestamp since which `peers_total === 0`. Bookkeeping only (`sync()` per poll tick, mexendo só nos hashes da lista recebida — `AnimeDetail` sincroniza um anime só); the 10-minute "sem seeds" threshold is a chip rule and lives in `deriveAnimeChip`. In-memory, resets on reload |
...
The same anime linked on multiple accounts appears once per account in the merged list, each with its own `MediaList.Id`/`Progress`/`Status` but the same `Media` (and the same airing-schedule episode IDs, which key every download/keep/delete decision). **`Media.Id` — not `MediaList.Id` — is therefore this app's anime identity**: it is what `episodes.json`, `anime_settings`, the `/animes/{id}/*` routes and the `anilist.co` link all use ([decisions.md #43](decisions.md#43-a-identidade-de-um-anime-é-mediaid-status-é-uma-pergunta-por-conta-download--or-deleção--and)). Installations predating that are converted once by `daemon.MigrateAnimeIDsToMedia`, gated by the `anime_ids_are_media_ids` config flag; the verification pass aborts until it succeeds.
