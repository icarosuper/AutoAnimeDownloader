...
| `findCoveringPack(saved, seriesIndex, hashSet, series, absEp)` | (`coverage.go`) O registro de pack, em **qualquer** `anime_id`, cuja faixa declarada (`hasDeclaredRange`) contém `absEp`. Quatro portas: faixa conhecida, torrent vivo na sessão, mesma `Series.Key`, e o anime pedido não ser filme |
