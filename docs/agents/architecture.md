...
| `GET` | `/api/v1/animes` | `handleAnimes` | `endpoint_animes.go` — `AnimeInfo.is_standalone` marks animes tracked via `standalone_animes`; `next_airing_at` (unix seconds, omitted when there's no scheduled episode) feeds the "next episode in X" chip |
