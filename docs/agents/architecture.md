...
- **No `config.json` versioning.** There is no `config_version` field, no migration chain and no
  startup block: `LoadConfigs` unmarshals **over** `getDefaultConfig()`, so an added or removed field
  needs no migration at all, and the two legacy renames are detected by **shape**, in `LoadConfigs`
  only — `PUT /config` does not repeat them. See decisions.md #90.
...
| `GET` | `/api/v1/animes` | `handleAnimes` | `endpoint_animes.go` — `next_airing_at` é **timestamp**, não contagem regressiva: a resposta é cacheada 60s e um contador pré-computado congelaria. Busca por `alt_names` é client-side (`lib/utils/status.ts`). `episodes_downloaded` é **arquivo em disco agora**, não acumulado: ele CAI quando o daemon poda assistido, então `released - downloaded` não dá "falta baixar" — quem dá é `episodes_pending` ([decisions.md #88](decisions.md#88-a-barra-de-progresso-deriva-baixados-de-episodes_pending-e-não-de-episodes_downloaded)) |
