...
| `elementFromPoint` over a MoreMenu item returns the menu, not page content (on `#/priorities`, whose cards were `position: relative` via daisyUI's `.card` — daisyUI is gone, but the rail's `z-30` still guards the general case) | the rail losing `z-30`: `position: sticky` creates a stacking context, so the panel's `z-50` only orders things *inside* the rail |
...
| every config group and exit link passes `toBeInViewport({ ratio: 1 })` and the index has `scrollWidth <= clientWidth` at 375/390/414px | the Config side index going back to `overflow-x-auto`, which hid "Torrent search" and both exit links (decision 39) |
| the three Status daemon actions pass `toBeInViewport({ ratio: 1 })` and the page does not scroll horizontally at 320/375/390/414px **in both `en` and `pt-BR`** | the action row going back to `flex shrink-0` with no `flex-wrap`, which pinned it to the sum of its buttons and pushed "Parar Daemon" off-screen |

**Width tests must run in `pt-BR`, not only the default `en`.** Set it before navigating: `page.addInitScript(() => localStorage.setItem('PARAGLIDE_LOCALE', 'pt-BR'))`. pt-BR labels are the longer of the two — measured, the Status daemon-action row is 358px in `en` and 438px in pt-BR. That 80px is the difference between a bug that reproduces below 320px (outside every width these tests use) and one that reproduces at 414px, on a phone. A width test that runs only in `en` measures the easy case; this exact gap let the Status header bug ship. Only parametrize by locale where **label length drives geometry** — for stacking, sticky and scroll-container assertions it is pure runtime.
