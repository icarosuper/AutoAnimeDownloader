<script lang="ts">
  ...
  import { getStatus, getAnimes, triggerCheck, startDaemon, stopDaemon, type StatusResponse, type AnimeInfo } from '../lib/api/client.js'
  ...
  async function loadData() {
    try {
      ...
      status = statusData
      // Sort animes by latest episode ID and take top 10
      animes = animesData
        .sort((a, b) => b.latest_episode_id - a.latest_episode_id)
        .slice(0, 10)
    } catch (err) {
...
<div>
  <div class="mb-6">
    ...
    <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Daemon monitoring and statistics</p>
  </div>
  ...
  {:else if status}
    ...
    <div class="bg-white dark:bg-gray-800 shadow rounded-lg mb-6">
      <div class="px-4 py-5 sm:p-6">
        <div class="flex items-center justify-between mb-4">
          <h2 class="text-lg font-medium text-gray-900 dark:text-white">Daemon Status</h2>
        ...
        <dl class="grid grid-cols-1 gap-5 sm:grid-cols-3 mt-5">
          ...
          <div class="px-4 py-5 bg-gray-50 dark:bg-gray-700 rounded-lg">
            <dt class="text-sm font-medium text-gray-500 dark:text-gray-400">Downloaded Animes</dt>
            <dd class="mt-1 text-lg font-semibold text-gray-900 dark:text-white">
              {animes.length}
            </dd>
          </div>
          <div class="px-4 py-5 bg-gray-50 dark:bg-gray-700 rounded-lg">
            <dt class="text-sm font-medium text-gray-500 dark:text-gray-400">Total Episodes</dt>
            <dd class="mt-1 text-lg font-semibold text-gray-900 dark:text-white">
        ...
        {#if status.has_error}
          <div class="mt-4 rounded-md bg-yellow-50 dark:bg-yellow-900/20 p-4 border border-yellow-200 dark:border-yellow-800">
            <div class="flex">
        ...
        <div class="mt-6 flex space-x-3">
          ...
          >
            {actionLoading ? 'Checking...' : 'Force Check'}
          </button>
        </div>
      </div>
    </div>
    ...
    <div class="bg-white dark:bg-gray-800 shadow rounded-lg">
      <div class="px-4 py-5 sm:p-6">
        <h2 class="text-lg font-medium text-gray-900 dark:text-white mb-4">Latest Downloaded Animes</h2>
        {#if animes.length === 0}
          <p class="text-sm text-gray-500 dark:text-gray-400">No animes downloaded yet.</p>
        {:else}
          <!-- Desktop Table View -->
              ...
              <div class="bg-gray-50 dark:bg-gray-700 rounded-lg p-4">
                ...
                <div class="grid grid-cols-2 gap-4 mt-3">
                  <div>
                    ...
                    <p class="text-sm font-medium text-gray-900 dark:text-white">{anime.episodes_count}</p>
                  </div>
                  <div>
                    <p class="text-xs text-gray-500 dark:text-gray-400">Latest Episode</p>
                    <p class="text-sm font-medium text-gray-900 dark:text-white">{anime.latest_episode_id}</p>
                  </div>
                </div>
              </div>
            {/each}
